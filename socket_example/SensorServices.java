...
//Endof Options
    ...
    private static final float _PI_ = (float) Math.PI;
      ...
      public void sent_orieation(String ip)   //新加发送函数
    {
        ...
        try
        {
            ...
            while(true)
           {
            ...
            updateOrientation();
            for(int i=0;i<3;i++)
                toserver.writeFloat(prevOrient[i]);
            toserver.flush();
           }
    ...
    private float[] calculateOrientation()   //返回orientation   (yaw,pitch,roll)
    {
        float[] tmpOrient = new float[3];
        SensorManager.getOrientation(rotationMx, tmpOrient);
    ...
    private float[] getRotationMxFromOrientation(float[] o) {
        ...
        //参考： https://blog.csdn.net/loongkingwhat/article/details/82765388?utm_medium=distribute.pc_relevant.none-task-blog-2%7Edefault%7ECTRLIST%7Edefault-2.no_search_link&depth_1-utm_source=distribute.pc_relevant.none-task-blog-2%7Edefault%7ECTRLIST%7Edefault-2.no_search_link
        //   https://blog.csdn.net/weixin_39654751/article/details/111584053?utm_medium=distribute.pc_relevant.none-task-blog-2~default~baidujs_baidulandingword~default-4.no_search_link&spm=1001.2101.3001.4242
        float sinX = (float) Math.sin(o[1]);  //o[1]为绕x旋转的角度   pitch
        float cosX = (float) Math.cos(o[1]);
        float sinY = (float) Math.sin(o[2]);   //o[2]为绕y旋转的角度   roll
        float cosY = (float) Math.cos(o[2]);
        float sinZ = (float) Math.sin(o[0]);  //o[0]为绕z旋转的角度   yaw
